  ...
  return (
    <Box m={4} p={3} bg="bg.primary" borderRadius={2} boxShadow="2px 2px #eee" minHeight={640}>
      <SubNav aria-label="Main" mb={3} display="flex">
        ...
        {historyMode && <TextInput type="datetime-local" value={endTime} onChange={(e) => setEndTime(e.target.value)}
                                   style={{height: 20}} width={230}/>}
      ...
      <Box display="flex">
        ...
        <Box width="400">
          <RequestDetail data={selectedRequest && selectedRequest.data} user={selectedRequest && selectedRequest.user}/>
        </Box>
      </Box>
    </Box>
  );
      ...
      <SelectMenu.Modal width={110}>
        <SelectMenu.List>
          {Object.entries(options).map(([index, name]) => {
            return (<SelectMenu.Item key={index} href="#"
                                     onClick={() => onChange && onChange(index)}>{name}</SelectMenu.Item>)
          })}
        </SelectMenu.List>
      </SelectMenu.Modal>
    </SelectMenu>
